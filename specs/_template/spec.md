...
- **Performance**: [e.g., conversion completes synchronously, playback latency < 50ms]
- **Security**: [e.g., no network access, local-only data storage]
- **Accessibility**: [e.g., content descriptions on buttons, screen reader support]
- **Observability**: [e.g., errors surfaced as snackbar, playback state visible in UI]
- **Offline**: [e.g., fully offline — no network requests]
